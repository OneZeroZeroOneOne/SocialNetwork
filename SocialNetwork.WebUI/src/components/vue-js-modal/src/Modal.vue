...
export default {
  ...
  methods: {
    ...
    ensureShiftInWindowBounds () {
      ...
      const maxLeft = 9999//viewport.width - trueModalWidth
      const maxTop = 9999//viewport.height - trueModalHeight

      const left = shift.left + pivotX * maxLeft
      const top = shift.top + pivotY * maxTop

      this.shift.left -= left - inRange(-9999, maxLeft, left)
      this.shift.top -= top - inRange(-9999, maxTop, top)
    }
  }
}
</script>
