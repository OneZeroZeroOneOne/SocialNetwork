...
using SocialNetwork.Markdown.Jsonize.Common;
using SocialNetwork.Markdown.Jsonize.Models;
using SocialNetwork.Markdown.Jsonize.Options;
using static System.Net.WebUtility;
using NullValueHandling = SocialNetwork.Markdown.Jsonize.Options.NullValueHandling;

namespace SocialNetwork.Markdown.Jsonize
{
    public class Jsonize
    {
        public HtmlDocument _htmlDoc;
        ...
        public JsonizeNode ParseHtmlToTypedJson()
        {
            JsonizeNode parentJsonizeNode = new JsonizeNode();
            HtmlNode parentHtmlNode = _htmlDoc.DocumentNode;
            Counter counter = new Counter();
        ...
        private void GetChildren(JsonizeNode parentJsonizeNode, HtmlNode parentHtmlNode, Counter counter)
        {
            foreach (HtmlNode htmlNode in parentHtmlNode.ChildNodes)
            {
                bool addToParent = false;
                JsonizeNode childJsonizeNode = new JsonizeNode();
                childJsonizeNode.NodeId = counter.Next();
                childJsonizeNode.ParentId = parentJsonizeNode.NodeId;
                ...
                if (_emptyTextNodeHandling == EmptyTextNodeHandling.Include || !string.IsNullOrWhiteSpace(innerText))
                {
                    ...
                    {
                        childJsonizeNode.Text = innerText != null && innerText.Equals("") ? null : innerText;
                    }
                ...
                if (htmlNode.HasChildNodes)
                {
                    GetChildren(childJsonizeNode, htmlNode, counter);
                    addToParent = true;
                }
